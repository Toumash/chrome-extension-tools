...
test(
  ...
  async () => {
    ...
    const fullReload = firstValueFrom(routes.pipe(timeout({ first: 5000 })))
    await update('header.ts')
    await fullReload

    await page.locator('h1', { hasText: header }).waitFor()
    expect(reloads).toBeGreaterThanOrEqual(1) // full reload on js update
    ...
    const runtimeReload = firstValueFrom(
      routes.pipe(timeout({ first: 15_000 })),
    )
    await update('bg-onload.ts')
    await Promise.all([runtimeReload, closeEvent])
